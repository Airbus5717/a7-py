...
def verify_example(
    ...
    c_out = build_dir / f"{example.stem}.c"
    obj_out = build_dir / f"{example.stem}.o"
    ...
    syntax_proc = run_cmd(
        ["zig", "cc", "-std=c11", "-c", str(c_out), "-o", str(obj_out)],
        cwd=ROOT,
    )
    if syntax_proc.returncode != 0:
        result.error = f"zig cc syntax check failed:\n{first_error_text(syntax_proc)}"
        return result
    result.syntax_ok = True

    build_proc = run_cmd(
        ["zig", "cc", "-std=c11", str(c_out), "-lm", "-o", str(bin_out)],
        cwd=ROOT,
    )
    if build_proc.returncode != 0:
        result.error = f"zig cc build failed:\n{first_error_text(build_proc)}"
        return result
...
def summarize(results: list[ExampleResult]) -> tuple[int, int]:
    ...
    print(f"Examples verified (C backend): {passed}/{total}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify examples end-to-end (C backend)")
    parser.add_argument(
        "--examples-dir",
    ...
    with tempfile.TemporaryDirectory(prefix="a7-e2e-c-") as tmp:
        build_dir = Path(tmp)
        results = [
            verify_example(
                example=example,
